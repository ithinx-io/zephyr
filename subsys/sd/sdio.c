	...
	uint32_t tuple_count)
{
	...
	for (int i = 0; i < 3; i++) {
		ret = sdio_io_rw_direct(func->card, SDIO_IO_READ, SDIO_FUNC_NUM_0,
			SDIO_FBR_BASE(func->num) + SDIO_FBR_CIS + i, 0, data + i);
		if (ret) {
			return ret;
		}
	}
	cis_ptr = *((uint32_t *)data);
	/* Read CIS tuples until we have read all requested CIS tuple codes */
	do {
		/* Read tuple code */
		ret = sdio_io_rw_direct(func->card, SDIO_IO_READ, func->num,
			cis_ptr++, 0, &tpl_code);
		if (ret) {
			return ret;
		}
...
{
	int ret, retries = CONFIG_SD_RETRY_COUNT;
	int timing = 0;
 ...
 */
int sdio_enable_func(struct sd_card *card, struct sdio_func *func,
	enum sdio_func_num num)
{
	...
	ret = sdio_read_cis(func, cis_tuples, ARRAY_SIZE(cis_tuples));
	if (ret) {
		return ret;
	}
...
int sdio_read_byte(struct sdio_func *func, uint32_t reg, uint8_t *val)
{
	...
	if (func->card->type == CARD_SDMMC) {
		LOG_WRN("MMC does not support SDIO commands");
		return -ENOTSUP;
	}
